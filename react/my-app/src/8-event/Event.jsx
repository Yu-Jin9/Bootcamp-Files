...
// <input type="submmit" />
// e.preventDefault(); => 이렇게 되면 위에 input이 submit이 설정되어 있어도 데이터가 날아가지 않음
...
export function Event() {
  ...
  return (
    <div style={style}>
      <h3>{message}</h3>
      {/* handleClick() 이렇게 작성하면 즉시 실행 되므로 주의 */}
...
export function Event2() {
  ...
  const handleClick = (e, id) => {
    // submit은 form태그 안에 있으면 data가 날아감
    // submit은 react에서는 잘 사용하지 않음
    e.preventDefault(); // 이벤트 기본 동작을 방지 (button의 type이  submit일 때, 통신이 날아가지 않도록)
  ...
  return (
    <div>
      <form action="http://myserver.com">
        <h3>{message}</h3>
        {/* 이벤트 함수는 event(e) 객체를 전달받을 수 있다. */}
        <button type="submit" onClick={(event) => handleClick(event, 1)}>
          first
        </button>
        <button onClick={(event) => handleClick(event, 2)}>second</button>
      </form>
    </div>
  );
}
