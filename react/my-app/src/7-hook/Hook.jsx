...
// App7.js inport * as E from "7-hook/Hook"
...
// 1. useEffect 훅
//     : 사이드 이펙트 관리 훅 (사이드 이펙트는 생명주기와 관련)
...
export function Counter3() {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => {
          // 한 줄 짜리 코드라서 중괄호와 세미클론 생략 가능(화살표 함수)
          setCount(count + 1);
        }}
      >
        증가
...
//     : 컴퍼넌트가 렌더링 될 때마다 매번 계산하는 대신 의존성 배열에 명시된 값이 변경될 때만 해당 연산을 다시 수행한다.
...
export function Counter4() {
  ...
  // number 상태 변수가 변경되지 않는 한, double값은 재 연산되지 않음
...
// 5. useRef 훅
//     : useRef는 리액트에서 변경 가능한 참조 객체를 제공하는 훅입니다.
...
export function Counter6() {
  ...
  // clickCountRef 컴포넌트가 리렌더링 될 때 값이 초기화 되지 않음
  // count는 리렌더링 시 화면에 즉시 반영하지만, clickCountRef는 리렌더링을 발생시키지 않고 값만 유지한다.
...
export function Counter7() {
  ...
  const handleClick = () => {
    ...
    if (inputRef.current.focus) {
      inputRef.current.value = `현재카운트 : ${count + 1}`;
