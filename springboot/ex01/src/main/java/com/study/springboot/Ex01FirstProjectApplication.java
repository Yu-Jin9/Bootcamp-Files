...
// 싱글톤이며 같은 이름의 빈은 선언불가, 첫글자는 영문소문자임. 에) studentScore

// 클래스를 빈을 만드는 어노테이션 2가지
// @Bean와  @Component의 차이
// 1. @Bean은 Config클래스 안에서 사용되고 , 주로 외부 라이브러리 사용시 (개발자 제어불가) 사용.
// 2. @Comoponent는 주로 개발자가 직접 작성한 클래스에 사용

@Configuration
class  MyConfig {	//웹 애플리케이션의 설정 정보를 담는 클래스 이다.
	@Bean
...
public class Ex01FirstProjectApplication {

	public static void main(String[] args) {

		SpringApplication.run(Ex01FirstProjectApplication.class, args);
