...
public class HtmlController {
    ...
    public String logoutAction(HttpSession session) {
        ...
        session.invalidate();   // 세션 종료 (JSESSIONID 값을 삭제)
    ...
    public String viewMember(@RequestParam int id, Model model) {
        ...
        if( !optional.isPresent()) {    // 아이디가 없을 경우
        ...
        return "modifyForm"; // 이렇게 바인딩해서 넘겨주면 됨
    }
    ...
    public String deletemember(@RequestParam int id) {
        ...
        try {
            ...
            e.printStackTrace();
            return "<script>alert('회원 삭제 실패');history.back();</script>";
        }
        return "<script>alert('회원 삭제 성공'); location.back();</script>";
    }
